	...
	for _, proto := range protocolConfigs {
		...
		if len(regUrls) > 0 {
			...
			if ivkURL.GetParam(constant.InterfaceKey, "") == constant.MetadataServiceName {
				...
				if err != nil {
					logger.Warnf("export org.apache.dubbo.metadata.MetadataService failed beacause of %s ! pls check if you import _ \"github.com/xd-luqiang/dubbo-go/v3/metadata/service/local\"", err)
					return nil
				}
